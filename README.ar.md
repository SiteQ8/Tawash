...
طوّاش أداة لرصد التهديدات تكشف النطاقات الشبيهة التي صُنعت لتبدو كأنها نطاقات الكويت الأصلية، إذ يكفي أن تعطيه نطاقاً فيبحث في نظام أسماء النطاقات عن النطاقات الشبيهة المسجلة التي تنتحله، سواء كانت أخطاء كتابة أو حروفاً مبدّلة أو حروفاً أجنبية أو كتابات لاتينية للأسماء العربية، كما يراقب القائمة اليومية للنطاقات المسجلة حديثاً وسجلات شفافية الشهادات بحثاً عن أسماء تستعير علامة رسمية كويتية.
...
ويعمل المحرك نفسه في المتصفح وفي سطر الأوامر، فلا تحتاج أداة البحث على الويب إلى أي خادم لأن متصفحك هو من يستعلم في نظام أسماء النطاقات، ويبحث طوّاش افتراضياً في مجموعة ثابتة من النهايات الشائعة ونهايات الخليج، أما مع الخيار `--all-tlds` أو الخيار المقابل في الصفحة فيبحث في كل النهايات المدرجة لدى IANA وعددها نحو 1400، وهذا يستغرق وقتاً أطول.

## الأوامر

### `scan <domain>`

يبحث في نظام أسماء النطاقات عن النطاقات الشبيهة المسجلة لنطاق ما ويقرأ تواريخ تسجيلها، ثم يمنح ما يجده درجة ويعرض ما يستحق النظر.
...
يسرد الأسماء التي يبحث عنها الأمر `scan` دون الاستعلام عن أي منها، وهذا مفيد لتمريرها إلى قائمة حظر أو إلى أداة أخرى.
...
هو التشغيل اليومي، إذ يشغّل `nrd` ثم `ct` مع الخيار `--ct` ثم يمسح نظام أسماء النطاقات بحثاً عن النطاقات الشبيهة بكل نطاق رسمي مع الخيار `--sweep`، فيتحقق من كل ما يجده ويكتب النتائج في مجلد بصيغتي JSON وMarkdown.
...
يسرد أساليب الانتحال التي يبحث عنها طوّاش مع سطر يشرح كلاً منها.
...
| `--algorithms <list>` | أساليب مفصولة بفواصل للأمرين `scan` و`candidates` |
...
| `--all-tlds` | يبحث في كل نطاقات المستوى الأعلى المدرجة لدى IANA وعددها نحو 1400، وهذا يستغرق وقتاً أطول |
...
| `--sweep` | يمسح الأمر `watch` أيضاً نظام أسماء النطاقات بحثاً عن النطاقات الشبيهة بكل نطاق رسمي |
...
| `wrong-tld` | نهاية أخرى من المستوى الأعلى بما فيها نطاقات الخليج وكل النهايات المدرجة لدى IANA مع الخيار `--all-tlds` | من `example.com` إلى `example.com.kw` أو `example.xyz` |
...
## تشغيل المراقبة يومياً

صُمّم الأمر `watch` ليعمل مهمةً مجدولة على جهاز تتحكم فيه، كما تُشغَّل أداة openSquat عبر `cron`:

</div>

```sh
# كل صباح عند 06:30 بعد نشر قائمة النطاقات المسجلة حديثاً
30 6 * * * cd /srv/tawash && npx --yes github:SiteQ8/Tawash#v1.1.0 watch --keywords watchlist.txt --ct --out reports
```

<div dir="rtl">

واحتفظ بالتقارير في مكان خاص، لأن الاسم الذي يستعير علامة ما ليس احتيالاً بالضرورة، ونشر اتهام لم يُراجَع ظلم لمالكه، ولهذا تبقى النطاقات المرشحة حيث يراجعها شخص أولاً، كما ينبغي تثبيت وسم الإصدار كما في المثال حتى تشغّل المهمة شيفرة قرأتها.
...
node scripts/sync-web.mjs --check  # الفحص نفسه الذي تشغّله مهمة التكامل المستمر
```
...
ولا يشغّل المستودع أي مهام مجدولة أو تلقائية، لذا لا تكلّف استضافته شيئاً، إذ الموقع ثابت ويجري كل بحث في متصفح الزائر، أما مهمة التكامل المستمر فلا تعمل إلا إذا شغّلها أحد يدوياً من تبويب Actions.

## إخلاء المسؤولية

طوّاش أداة مفتوحة المصدر للعرض والتجربة تُبيّن كيف تُكشف النطاقات الشبيهة، وتأتي نتائجها من بيانات نظام أسماء النطاقات وسجلات التسجيل العامة لحظة البحث، لذا قد تكون ناقصة أو قديمة ولا تُعد حكماً على أي نطاق أو على مالكه، كما تُقدَّم الأداة كما هي دون أي ضمان ولا يتحمل مطوروها أي مسؤولية عن استخدامها أو عن القرارات المبنية عليها، وذلك وفق [ترخيص MIT](LICENSE).
